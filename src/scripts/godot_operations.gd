...
# this variable is threaded through so the one unconditional quit() at the
# bottom of _init() always reports the right code.
var exit_code := 0
...
func _init():
    ...
    if args.size() <= params_index:
        fail("Usage: godot --headless --script godot_operations.gd <operation> <json_params>. Not enough command-line arguments provided.")
        quit(exit_code)
        return
    ...
    if error == OK:
        ...
        fail("Failed to parse JSON parameters: " + params_json + " (JSON Error: " + json.get_error_message() + " at line " + str(json.get_error_line()) + ")")
        quit(exit_code)
        return
...
## Print the JSON failure verdict on stdout, log the message to stderr with
## the existing [ERROR] convention, and mark the process to exit non-zero.
## Callers inside an operation function MUST `return` immediately after
## calling fail() to avoid running further logic against invalid state.
...
func create_scene(params):
    ...
    if not scene_root:
        fail("Failed to instantiate node of type: " + root_node_type + ". Make sure the class exists and can be instantiated (check if it is registered in ClassDB or available as a script).")
        return
    ...
    if result != OK:
        fail("Failed to pack scene. Error code: " + str(result))
        return
        ...
        if not dir_exists:
            ...
            if dir == null:
                ...
                if make_dir_error != OK:
                    fail("Failed to create directory using absolute path: " + scene_dir_abs + " (open error: " + str(open_error) + ", make_dir error: " + str(make_dir_error) + ")")
                    return
            else:
                ...
                if make_dir_error != OK:
                    fail("Failed to create directory: " + scene_dir_relative + ", error code: " + str(make_dir_error))
                    return
            ...
            if not dir_exists:
                fail("Directory reported as created but does not exist: " + scene_dir_abs)
                return
        elif debug_mode:
    ...
    if save_error != OK:
        ...
        var error_message = "Failed to save scene: " + params.scene_path + ". Error code: " + str(save_error)
    ...
    if not file_exists:
        fail("Scene reported as saved but file does not exist at: " + full_scene_path)
        return

    print("Scene created successfully at: " + params.scene_path)
    print(JSON.stringify({"success": true, "scene_path": params.scene_path, "root_node_type": root_node_type}))
...
func add_node(params):
    ...
    if not FileAccess.file_exists(absolute_scene_path):
        fail("Scene file does not exist at: " + absolute_scene_path)
        return
    ...
    if not new_node:
        fail("Failed to instantiate node of type: " + params.node_type + ". Make sure the class exists and can be instantiated (check if it is registered in ClassDB or available as a script).")
        return
    ...
    if result != OK:
        fail("Failed to pack scene: " + str(result))
        return
    ...
    print(JSON.stringify({"success": true, "node_name": params.node_name, "node_type": params.node_type}))
...
func load_sprite(params):
    ...
    if not file_check:
        ...
        fail("Scene file does not exist at: " + full_scene_path + " (absolute path: " + absolute_path + ")")
        return
    ...
    if not (sprite_node is Sprite2D or sprite_node is Sprite3D or sprite_node is TextureRect):
        fail("Node is not a sprite-compatible type: " + sprite_node.get_class() + " (node: " + params.node_path + ")")
        return
    ...
    if error != OK:
        fail("Failed to save scene: " + str(error))
        return
...
func export_mesh_library(params):
    ...
    var dir = DirAccess.open("res://")
    if dir == null:
        fail("Failed to open res:// directory. DirAccess error: " + str(DirAccess.get_open_error()))
        return
    ...
    if output_dir != "res://" and not dir.dir_exists(output_dir.substr(6)):  # Remove "res://" prefix
        ...
        if error != OK:
            fail("Failed to create directory: " + output_dir + ", error: " + str(error))
            return

    # Save the mesh library
    if item_id <= 0:
        fail("No valid meshes found in the scene: " + params.scene_path)
        return
    ...
    if error != OK:
        fail("Failed to save MeshLibrary: " + str(error))
        return
...
func get_uid(params):
    ...
    if not file_check:
        fail("File does not exist at: " + file_path + " (absolute path: " + absolute_path + ")")
        return
...
func save_scene(params):
    ...
    if params.has("new_path"):
        ...
        if scene_dir != "res://" and not dir.dir_exists(scene_dir.substr(6)):  # Remove "res://" prefix
            ...
            if error != OK:
                fail("Failed to create directory: " + scene_dir + ", error: " + str(error))
                return
    ...
    print(JSON.stringify({"success": true, "scene_path": save_path}))
...
func modify_node_property(params):
    ...
    if result != OK:
        fail("Failed to pack scene after modification: " + str(result))
        return
...
func remove_node(params):
    ...
    if node_path == "root" or node_path == "":
        fail("Cannot remove the root node")
        return
    ...
    if result != OK:
        fail("Failed to pack scene after removal: " + str(result))
        return
...
func attach_script(params):
    ...
    if result != OK:
        fail("Failed to pack scene after attaching script: " + str(result))
        return
